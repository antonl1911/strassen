/** Original by Martin Thoma
  https://github.com/MartinThoma/matrix-multiplication/blob
 **/
#include <sstream>
#include <string>
#include <fstream>
...
void strassen(vector< vector<data> > &A,
              vector< vector<data> > &B,
              vector< vector<data> > &C, unsigned int tam);
unsigned int nextPowerOfTwo(int n);
void strassenR(vector< vector<data> > &A,
              vector< vector<data> > &B,
              vector< vector<data> > &C,
              int tam);
void sum(vector< vector<data> > &A,
         vector< vector<data> > &B,
         vector< vector<data> > &C, int tam);
void subtract(vector< vector<data> > &A,
              vector< vector<data> > &B,
              vector< vector<data> > &C, int tam);

void printMatrix(vector< vector<data> > matrix, int n);
void read(string filename, vector< vector<data> > &A, vector< vector<data> > &B);

void ikjalgorithm(vector< vector<data> > A,
                                   vector< vector<data> > B,
                                   vector< vector<data> > &C, int n) {
    for (int i = 0; i < n; i++) {
        for (int k = 0; k < n; k++) {
	    const data aink = A[i][k];
            for (int j = 0; j < n; j++) {
                C[i][j] += aink * B[k][j];
            }
        }
...
void strassen(vector< vector<data> > &A,
              vector< vector<data> > &B,
              vector< vector<data> > &C, unsigned int n) {
    //unsigned int n = tam;
    unsigned int m = nextPowerOfTwo(n);
    vector<data> inner(m);
    vector< vector<data> > APrep(m, inner), BPrep(m, inner), CPrep(m, inner);
...
int main (int argc, char* argv[]) {
    ...
    vector<data> inner (n);
    vector< vector<data> > A(n, inner), B(n, inner), C(n, inner);
    read (filename, A, B);
    strassen(A, B, C, n);
    printMatrix(C, n);
    return 0;
